...
class ServerManager:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.config = Box(config)  # 加载关于工具的config，
    ...
    def run_local_command(self, job_name: str, command: List[str], log_file: str, 
        ...
        with open(log_file, 'w') as f: # 核心步骤：启动子进程：在后台启动一个新的 Python 进程来运行控制器，并将该子进程的标准输出和错误流重定向到指定的日志文件
    ...
    def start_controller(self) -> str:
        ...
        for k, v in self.controller_config.cmd.items():  # 准备启动工具的命令，在终端中执行
